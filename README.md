...
The backend server file, "server.js", employs the Node.js web application framework 'Express' for setting up a local server.  Note, for the purposes of sharing code as a public open source demo, setup steps for a dedicated backend datastore (e.g. database) are not included. These steps nevertheless still allow serving local testing and development purposes, allowing for you to choose your own data persistence solution (e.g. across server restarts/user sessions/devices etc). Functionality provided includes:
